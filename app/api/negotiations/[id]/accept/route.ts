...
export async function POST(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  if (neg.status !== "accepted") {
    return NextResponse.json(
      { error: `cannot human-approve a negotiation in status=${neg.status}` },
      { status: 400 },
    );
  }
  // Already auto-accepted by agents; human confirmation is a no-op for the hackathon.
  return NextResponse.json({ ok: true, status: neg.status });
}
